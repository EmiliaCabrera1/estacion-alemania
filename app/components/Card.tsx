    ...
    >
      <div className="flex flex-row gap-2 w-full">
        ...
        <div className={twMerge("flex flex-col flex-2", props?.tipo === 2 ? "order-2 text-right ml-auto" : "order-2 text-left mr-auto")}>
          ...
          <p className={twMerge("mt-1 text-[8px] font-normal drop-shadow-card", props?.tipo === 2 ? "mr-2" : "ml-2")}>
            {props.descripcion}
          </p>
        </div>
        <div className={twMerge("flex flex-col", props?.tipo === 2 ? "order-1" : "order-3")}>
          <h4 className={twMerge("text-[10px] font-medium drop-shadow-card whitespace-nowrap", props.descripcion ? "my-4" : "my-0")}>
            {formatARS(props.precio)}
          </h4>
          <div className="flex flex-row mt-auto gap-2 justify-end">
            {props.vegetariano && (
              <div className="relative flex h-4 w-4">
                <Image src="/icons/vegetarianoOscuro.svg" alt="vegetariano" fill />
              </div>
            )}
            {props.sinTacc && (
